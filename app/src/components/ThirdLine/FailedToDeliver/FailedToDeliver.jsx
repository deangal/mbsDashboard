...
export default function FailedToDeliver() {
...
useEffect(() => {
  FiltersData.forEach(element => {
    if(element.name == SelectedData){
      let result = element.data.filter( active => active == "FailedToDeliver0" || active == "FailedToDeliver1" || active == "FailedToDeliver2" )
      result.forEach( filter => {
        if(filter == "FailedToDeliver0"){
          active[0] = true
          setActive([...active])
        }

        if(filter == "FailedToDeliver1"){
          active[1] = true
          setActive([...active])
        }

        if(filter == "FailedToDeliver2"){
          active[2] = true
          setActive([...active])
        }
  ...
  if(DomainData.domain === "mbs"){
    ...
    if(active[DomainData.domainNumber] == true){
      dispatch(addFilterData({name: SelectedData, data: "FailedToDeliver" + DomainData.domainNumber}))
    }
    if(active[DomainData.domainNumber] == false){
      dispatch(removeFilterData({name: SelectedData, data: "FailedToDeliver" + DomainData.domainNumber}))
