...
export default function ReadyForDelivery({domain}) {
...
useEffect(() => {
  FiltersData.forEach(element => {
    if(element.name == SelectedData){
      let result = element.data.filter( active => active == "ReadyForDelivery0" || active == "ReadyForDelivery1" || active == "ReadyForDelivery2" )
      result.forEach( filter => {
        if(filter == "ReadyForDelivery0"){
          active[0] = true
          setActive([...active])
        }

        if(filter == "ReadyForDelivery1"){
          active[1] = true
          setActive([...active])
        }

        if(filter == "ReadyForDelivery2"){
          active[2] = true
          setActive([...active])
        }
  ...
  const addHandle = (e) =>{
    if(DomainData.domain === "mbs"){
      ...
      if(active[DomainData.domainNumber] == true){
        dispatch(addFilterData({name: SelectedData, data: "ReadyForDelivery" + DomainData.domainNumber}))
      }
      if(active[DomainData.domainNumber] == false){
        dispatch(removeFilterData({name: SelectedData, data: "ReadyForDelivery" + DomainData.domainNumber}))
